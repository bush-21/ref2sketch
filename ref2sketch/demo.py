...
parser.add_argument('--image_dir', type=str, required=True, help='input_image path')
parser.add_argument('--style_image_dir', type=str, required=True, help='style_image path')
parser.add_argument('--out_dir', type=str, required=True, help='out_image path')
...
style_test_image_dir = opt.style_image_dir
...
for style_name in style_test_image_dir_name:
    ...
    for image_name in image_filenames:
        ...
        if not os.path.exists(opt.out_dir + style_name):
            os.makedirs(os.path.join("{}/{}".format(opt.out_dir, style_name)))
        print(image_name)
        save_img(out_img, w, h, opt.out_dir + '/' + style_name + '/' + image_name)
